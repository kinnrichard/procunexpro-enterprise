...
import {
  ...
  Clock, FileText, Link2, Ban, ChevronRight, Building2, Calendar, Hash, CreditCard, MapPin,
} from 'lucide-react';
...
function POStatusTimeline({ status, rejectionNote }: Readonly<{ status: string; rejectionNote?: string }>) {
  ...
  const isRejected = status === 'REJECTED' || status === 'PARTIALLY_RECEIVED';
        ...
        return (
          <div key={step.key} className="flex items-center">
            <div className={cn(
              ...
              isDone ? doneChip : isCurrent ? colors.chip : 'bg-muted text-muted-foreground'
            )}>
              <Icon className="h-3.5 w-3.5" />
              <span className="hidden sm:inline">{step.label}</span>
            </div>
            {i < statusSteps.length - 1 && (
              <ChevronRight className={cn('h-4 w-4 mx-0.5 shrink-0', isDone ? doneArrow : isActive ? colors.arrow : 'text-muted-foreground/30')} />
            )}
          </div>
        );
      })}
...
export default function PurchaseOrderDetailPage() {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => { invalidate(); toast({ title: 'Purchase order approved' }); },
    onError: () => toast({ title: 'Failed to approve', variant: 'destructive' }),
  });

  const sendMutation = useMutation({
    mutationFn: () => api.put(`/purchase-orders/${poId}/send`),
    onSuccess: () => { invalidate(); toast({ title: 'Purchase order sent to vendor' }); },
    onError: () => toast({ title: 'Failed to send', variant: 'destructive' }),
  });

  const receiveMutation = useMutation({
    mutationFn: () => api.put(`/purchase-orders/${poId}/receive`),
    onSuccess: () => { invalidate(); toast({ title: 'Purchase order received' }); },
    onError: () => toast({ title: 'Failed to receive', variant: 'destructive' }),
  });
  ...
  function renderHeaderActions() {
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        {po.status === 'APPROVED' && (
          <Button onClick={() => setSendConfirmOpen(true)} disabled={sendMutation.isPending} className="bg-blue-600 hover:bg-blue-700 text-white">
            <Truck className="h-4 w-4 mr-2" /> Send to Vendor
          </Button>
        )}
        {(po.status === 'SENT' || po.status === 'PARTIALLY_RECEIVED') && (
          <Button onClick={() => setReceiveConfirmOpen(true)} disabled={receiveMutation.isPending} className="bg-emerald-600 hover:bg-emerald-700 text-white">
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              ...
              {po.purchaseRequest && <span> · PR {po.purchaseRequest.requestNumber}</span>}
            </p>
          </div>
        </div>
        {renderHeaderActions()}
        ...
        <TabsContent value="details" className="mt-5 space-y-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="text-right shrink-0">
              ...
              <p className="text-2xl font-bold font-mono text-primary">{formatCurrency(po.totalAmount || 0)}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-8 gap-y-4">
            ...
            {po.purchaseRequest && (
              <div className="space-y-1">
                ...
                <button onClick={() => router.push(`/purchase-requests/${po.purchaseRequest.id}`)} className="text-sm font-medium text-primary hover:underline flex items-center gap-1.5">
                  <Link2 className="h-3.5 w-3.5" />{po.purchaseRequest.requestNumber}
                </button>
              </div>
            )}
            <div className="space-y-1">
              <p className="text-[11px] uppercase tracking-wider text-muted-foreground font-medium">Priority</p>
              <span className={cn('inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium', priorityColors[po.priority])}>{po.priority}</span>
            </div>
            <div className="space-y-1">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-muted/50 text-muted-foreground text-[10.5px] uppercase tracking-wider">
                      ...
                      <th className="text-center px-3 py-2.5 w-[80px]">Received</th>
                  ...
                  <tbody>
                    {po.items.map((item: any, idx: number) => (
                      <tr key={item.id} className="border-t border-border/50">
                        ...
                        <td className="px-3 py-3 text-right font-mono">{formatCurrency(item.unitPrice || 0)}</td>
                        <td className="px-3 py-3 text-right font-mono font-medium">{formatCurrency(item.totalPrice || 0)}</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot className="border-t-2 border-border">
                    <tr>
                      ...
                      <td className="px-3 py-2 text-right font-mono font-medium">{formatCurrency(po.subtotal || 0)}</td>
                    </tr>
                    {(po.taxAmount || 0) > 0 && (
                      <tr>
                        <td colSpan={5} className="px-3 py-1 text-right text-sm text-muted-foreground">Tax</td>
                        <td className="px-3 py-1 text-right font-mono">{formatCurrency(po.taxAmount)}</td>
                      </tr>
                    )}
                    {(po.shippingCost || 0) > 0 && (
                      <tr>
                        <td colSpan={5} className="px-3 py-1 text-right text-sm text-muted-foreground">Shipping</td>
                        <td className="px-3 py-1 text-right font-mono">{formatCurrency(po.shippingCost)}</td>
                      </tr>
                    )}
                    <tr className="border-t">
                      <td colSpan={5} className="px-3 py-2 text-right text-sm font-semibold">Total</td>
                      <td className="px-3 py-2 text-right font-mono font-bold text-primary">{formatCurrency(po.totalAmount || 0)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:h-[500px]">
            <div className="flex flex-col gap-4 min-h-0">
              ...
              <div className="flex-1 min-h-0">
                {po.id && <CommentsPanel entityType="PURCHASE_ORDER" entityId={po.id} />}
              </div>
            </div>
            <div className="flex flex-col gap-4 min-h-0">
              <h2 className="text-base font-semibold shrink-0">Activity</h2>
              <div className="flex-1 min-h-0">
                {po.id && <ActivityPanel entityType="PURCHASE_ORDER" entityId={po.id} />}
              </div>
            </div>
          </div>
        </TabsContent>
      ...
      <ConfirmDialog
        ...
        description={`Submit "${po.orderNumber}" with ${po.items?.length || 0} item(s) for approval?`}
        confirmLabel="Submit"
      ...
      <ConfirmDialog
        ...
        title="Approve Order"
        description={`Approve purchase order "${po.orderNumber}"?`}
      ...
      <ConfirmDialog
        ...
        description={`Send "${po.orderNumber}" to ${po.vendor?.name || 'vendor'}?`}
        confirmLabel="Send"
        onConfirm={() => { sendMutation.mutate(undefined, { onSuccess: () => setSendConfirmOpen(false) }); }}
        isLoading={sendMutation.isPending}
      />

      <ConfirmDialog
        open={receiveConfirmOpen}
        onOpenChange={(open) => !open && setReceiveConfirmOpen(false)}
        title="Mark as Received"
        description={`Mark "${po.orderNumber}" as fully received?`}
        confirmLabel="Receive"
        onConfirm={() => { receiveMutation.mutate(undefined, { onSuccess: () => setReceiveConfirmOpen(false) }); }}
      ...
      <Dialog open={rejectConfirmOpen} onOpenChange={(open) => { if (!open) { setRejectConfirmOpen(false); setRejectionNote(''); } }}>
        <DialogContent className="max-w-xs p-5">
          ...
          <p className="text-sm text-muted-foreground mt-1.5">Reject purchase order &quot;{po.orderNumber}&quot;?</p>
          ...
          <div className="mt-4 flex justify-end gap-2">
            ...
            <Button size="sm" variant="destructive" onClick={() => toast({ title: 'Reject not yet implemented' })} disabled={!rejectionNote.trim()}>
              Reject
            </Button>
          </div>
        </DialogContent>
